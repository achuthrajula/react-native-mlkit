...
class PantryFull extends Component {
    ...
    render() {
        ...
        const vessel = [
            ...
            {title: "Fruits", source: require("../../assets/pantry/categories/fruit.png"), state: this.state.fruits, toggle: () => this.toggleFruits(),
                dish: [
            ...
            {title: "Spices", source: require("../../assets/pantry/categories/spice.png"), state: this.state.spices, toggle: () => this.toggleSpices(),
                dish: [{ title: "N/A", source: require('../../assets/pantry/categories/spice.png'), quantity: "N/A"}]},
            {title: "Liquids", source: require("../../assets/pantry/categories/liquid.png"), state: this.state.liquids, toggle: () => this.toggleLiquids(),
                dish: [{ title: "N/A", source: require('../../assets/pantry/categories/liquid.png'), quantity: "N/A"}]},
            {title: "Meats", source: require("../../assets/pantry/categories/meat.png"), state: this.state.meats, toggle: () => this.toggleMeats(), 
            dish: [
            ...
            {title: "Dairy", source: require("../../assets/pantry/categories/dairy.png"), state: this.state.dairy, toggle: () => this.toggleDairy(),
    ...
    return (
      <View style={{flex: 1, flexDirection: 'column'}}>
        ...
        <View style={{ flexDirection: 'row' }}>
            <ScrollView>
        {vessel.map(dish => { return(<View key={dish.title} style={{ flex: 1, justifyContent: 'flex-start', alignItems: 'center' }}>
            {dish.state && <Dialog style={{ marginBottom: 100 }}>
                <Dialog.Title><Text style={{ fontWeight: "bold" }}>{dish.title}</Text></Dialog.Title>
                <Dialog.Content>
                    {dish.dish.map(itr => {return (<View key={itr.title} style={{ flexDirection: 'row', height: 60 }}>
